...
Auf der X-Achse -14.54 und 0 auf der Z-Achse.


Ball:
Der Ball soll zufällig im FOV Radius Feld der Kamera platziert werden. Die Höhe Y-Achse bleibt immer gleich auf 0.034.
Zuerst wird nur der Z-Achsen-Wert, also die Entfernung verändert.
X-Achse bleibt -14.5. Der Ball befindet sich also in einer Linie mit der Kamera auf der X-Achse.
Der vorerst minimal einnehmbare Z-Achsen Wert ist 0.5, der maximale Wert ist 6.5.
Der Ball soll auf der Y-Achse bei jeder Platzierung um einen zufälligen Wert gedreht werden, was die Platzierung des Balls annähernd realistischer macht.

Ich habe ein C-Sharp Script implementiert, dass beim Starten der Unity Scene im Game Window den Ball auf einer Zufälligen entfernung paltziert und zufällig rotiert.
Für jeden Positionswechsel wird ein screenshot der neuen Position aufgenommen und die Distanz zur Kamera berechnet.
Dann wird der Screenshot gespeichert und zusammen mit dem Namen des Screenshots, die Distanz des Balls in diesem Screenshot in einer Liste gespeichert.
Diese Liste wird, wenn so viele Screenshots wie angegeben erstellt wurden in ein JSON umgewandelt.

Problematiken:
- verschwommene Tennisbälle, weil das neu positionieren des Tennisballs gleichzeitig mit der Aufnahme des Screenshots stattfand. Lösung: einige Frames nach der Transformation abwarten.
- (Unity erstellt für jede Datei eine meta Datei)

Das script habe ich dann an einem Objekt der Unity Szene hinzugefügt, wodurch es beim Start der Szene ausgeführt wird.
#figure(
  image("../assets/unitySetup/KameraScriptParameterInput.png", width: 70%),
  caption: [
    Inputmaske der Scriptparameter für das Aufnehmen der Screenshots in Unity.
  ],
)<script-inputmaske>

Wie in @script-inputmaske zu sehen können verschiedene Parameter für das Script vor dem Start eingestellt werden. Relevant ist hier jedoch nur, dass die Anzahl der aufzunehmenden Screenshots bestimmt werden kann.

=== Durchführung des gewählten Ansatzes
Die aufgenommenen Screenshots orientieren sich in ihrer Größe an der eingestellten Auflösung des Gamewindows in Unity.
Da die Objekterkennung später mit YOLO geschehen soll, habe ich die Displaygröße auf 640x640 Pixel gestellt. Das ist die empfohlene Auflösung für YOLO inputs.

Dann gebe ich die gewünschte Anzahl an Screenshots im Screenshot Count Parameter ein und starte die Unity Szene.

Die Screenshots werden nach jeder Platzierung des Balls automatisch aufgenommen und zusammen mit den Annotationen für die Distanz in einem Ordner mit dem aktuellen Datum und der aktuellen Uhrzeit als Namenskombiantion gespeichert.
